...
export default function CostSimulator(){
  ...
  return(
    <div style={{background:"#fff",borderRadius:12,padding:"20px 24px",border:"1px solid #e2e8f0"}}>
      ...
      {/* ── テーブル ── */}
      <div style={{overflowX:"auto",overflowY:"auto",maxHeight:"calc(100vh - 120px)",border:"1px solid #e2e8f0",borderRadius:8}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead style={{position:"sticky",top:0,zIndex:5}}>
            <tr style={{background:"#f8fafc",borderBottom:"2px solid #e2e8f0"}}>
              ...
              <SortH k="brand"><span style={{display:"block",fontSize:9,color:"#64748b",background:"#eff6ff",borderRadius:3,padding:"1px 4px",marginBottom:2,fontWeight:400}}>青背景は高額療養費適用</span>{isTest?"自己負担":`先発品 自己負担${months}ヶ月`}</SortH>
              ...
              return(
                <tr key={r.id} style={{borderBottom:"1px solid #f1f5f9"}}>
                  ...
                  <td style={{padding:"6px 8px",textAlign:"right",fontVariantNumeric:"tabular-nums",background:r.brand.applied?"#eff6ff":"transparent"}}>
                    ...
                    {!isTest&&<div style={{fontSize:10,color:"#94a3b8"}}>初月{fmt(r.brand.first)}{r.brand.multi>0&&` → 多数回${fmt(r.brand.multi)}/月`}</div>}
                  </td>
                  {!isTest&&showGE&&<td style={{padding:"6px 8px",textAlign:"right",fontVariantNumeric:"tabular-nums",background:r.ge?.applied?"#f0fdf4":"transparent"}}>
                    {r.ge?<>
      ...
      <div style={{marginTop:20,padding:"16px",background:"#f8fafc",borderRadius:8,border:"1px solid #e2e8f0"}}>
        ...
        <div style={{marginTop:12,padding:"10px",background:"#fef2f2",borderRadius:6,border:"1px solid #fecaca"}}>
          <p style={{fontSize:10,color:"#991b1b",margin:0,lineHeight:1.6}}>
            ...
            ・<strong>多数回該当について:</strong> 直近12ヶ月で高額療養費の対象が3回以上ある場合、4回目以降はさらに低い限度額（「多数回」の金額）が適用されます。長期治療では自己負担がさらに下がります。<br/>
            ・70歳以上の「一般」「住民税非課税」区分は外来のみの上限額を適用しています。入院がある場合は世帯合算の上限額が適用されます。<br/>
            ・70歳以上でも現役並み所得者（年収約370万円以上）は3割負担です。<br/>
            ・75歳以上の「一定以上所得」区分（年収約200〜370万円）は2割負担です。<br/>
            ・限度額適用認定証を事前に取得しておくと、窓口での支払いが限度額までに抑えられます。詳細は加入の健康保険にご確認ください。
          </p>
        </div>
      </div>
    </div>
