...
| **E. EBC（早期乳癌）Phase III** | MBCだけでなく、EBC設定（neoadjuvant/adjuvant/post-neoAdj）のPhase III試験も収録対象。2024年以降にreadoutがある、または進行中のもの。 | DESTINY-Breast05 (post-neoAdj T-DXd), DESTINY-Breast11 (neoadj T-DXd), lidERA (adjuvant giredestrant), monarchE OS等 |

> ⚠️ **収録漏れ頻発パターン（必ず確認）**
> 1. **EBC試験の独立収集**: EBCはMBCと別のカテゴリとして意識的に検索する（MBC中心の検索では漏れる）
> 2. **維持療法（maintenance）試験**: 「1st-line maintenance」「adjuvant maintenance」等の設定は標準的なライン分類に当てはまりにくく、見落とし注意（例：HER2CLIMB-05, PATINA）
> 3. **試験シリーズの連番チェック**: 収録済み試験がシリーズ名を持つ場合（DESTINY-Breast XX, ASCENT-XX, HER2CLIMB-XX等）、他の番号の試験が存在しないか必ずClinicalTrials.govで確認する
> 4. **Negative結果も収録**: 一次エンドポイントを達成しなかった試験も収録する（例：ASCENT-07, TROPION-Breast01のOS）。Negativeであることが重要な臨床情報となる
> 5. **既存薬の組み合わせ試験**: 新規薬剤がない試験（例：tucatinib + HP）も、Phase IIIでPositiveならば収録対象
...
### EBC Phase III 試験の必須確認リスト（漏れ防止）

以下のカテゴリについて、2024年以降にreadoutがある試験を確認する：

**HER2+ EBC**
- post-neoAdj（術後補助）: DESTINY-Breast05（T-DXd vs T-DM1）、Katherine（T-DM1 historical）
- neoadjuvant（術前）: DESTINY-Breast11（T-DXd-THP）
- adjuvant（術後補助）: COMPASS HER2（EA1181）、進行中試験
- maintenance: HER2CLIMB-05（tucatinib+HP）、PATINA（palbociclib+HP）

**HR+/HER2- EBC**
- adjuvant SERD: lidERA（giredestrant）、EMBER-4（imlunestrant）、CAMBRIA-1（camizestrant）、ELEGANT（elacestrant）
- adjuvant CDK4/6i: monarchE（abemaciclib OS）、NATALEE（ribociclib OS）

**TNBC EBC**
- neoadjuvant: KEYNOTE-522（pembro、OS確認）
- adjuvant: OlympiA（olaparib）、CREATE-X（capecitabine、日本のみ特記）
- 新規試験: SCARLET、CITRINE、RJBC-1501

> 注: このリストは完全ではない。上記のAgent 3専用EBC検索クエリで補完すること。
...
3. **学会発表（MBC）**: `[薬剤名] ASCO 2025` / `SABCS 2025` / `ESMO 2025`
4. **学会発表（EBC）**: `[薬剤名] early breast cancer neoadjuvant adjuvant ASCO ESMO SABCS 2024 2025`
5. **論文掲載**: `[薬剤名] breast cancer NEJM Lancet JCO 2025 2026`
6. **シリーズ連番確認**: 収録済み試験がXX-01〜XXシリーズの場合、`[シリーズ名] phase 3 complete list` でシリーズ全体を確認

> ⚠️ **EBC試験の専用検索（必須）**:
> 以下のクエリを別途独立して実行する（MBC中心の検索では漏れるため）:
> - HER2+ EBC全般: `HER2 positive early breast cancer phase 3 2024 2025 results`
> - T-DXd EBC: `trastuzumab deruxtecan early breast cancer neoadjuvant adjuvant DESTINY-Breast`
> - HR+/HER2- EBC: `HR positive early breast cancer adjuvant SERD CDK4/6 giredestrant abemaciclib phase 3 2025`
> - TNBC EBC: `triple negative early breast cancer neoadjuvant adjuvant phase 3 2024 2025`
> - EBC OS readout: `early breast cancer overall survival phase 3 ESMO SABCS 2024 2025`

> ⚠️ **維持療法（maintenance）試験の専用検索（必須）**:
> - HER2+ 維持療法: `HER2 positive breast cancer maintenance therapy phase 3 2024 2025`
> - HR+/HER2- 維持療法: `HR positive breast cancer maintenance CDK4/6 phase 3`
> - 全般: `breast cancer maintenance therapy randomized phase 3 results 2024 2025`

> ⚠️ **試験シリーズ連番チェック（必須）**:
> - DESTINY-Breast: `DESTINY-Breast 01 02 03 04 05 06 07 08 09 10 11 12 breast cancer`
> - ASCENT: `ASCENT sacituzumab govitecan phase 3 breast cancer all trials`
> - HER2CLIMB: `HER2CLIMB tucatinib phase 3 all trials 01 02 03 04 05`
> - TROPION-Breast: `TROPION-Breast datopotamab deruxtecan all trials`
> - TroFuse: `TroFuse sacituzumab tirumotecan breast cancer all trials`

> **収録判定ルール（Negative結果も含める）**:
> 以下のいずれかに該当すれば収録対象:
> - Phase IIIで一次エンドポイントを達成（Positive）
> - Phase IIIで一次エンドポイントを達成しなかったが（Negative）、二次エンドポイントに重要データあり
> - Phase IIIで進行中（readout待ち）
> - Phase IIIでNegative結果であり、かつその試験の薬剤が他の適応では収録されている
> Negative試験の表示: `st: "neg"` でマークし、「主要エンドポイント未達」と明記する。
...
- [ ] **EBC試験の独立確認**: EBC設定（neoadjuvant/adjuvant/post-neoAdj）のPhase IIIで2024年以降にreadoutがある試験が少なくとも3件以上含まれているか。0件の場合はAgent 3のEBC専用検索を必ず再実行。
- [ ] **シリーズ連番チェック完了**: 収録済みシリーズ（DESTINY-Breast, ASCENT, HER2CLIMB等）について、他の番号の試験がないか確認済みか。
